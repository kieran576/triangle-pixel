...
\begin{proof}
Empirically verified on all interior vertices (72/72, 10$\times$18 grid) and across 5 Sierpinski subdivision levels (11,346 hexagonal groups at 100\% accuracy). The period-6 modular arithmetic of the channel assignment guarantees this property.
\end{proof}
...
Each triangle face can map to a 3D mesh face when depth information is available (e.g., from a depth sensor or stereo matching). The shared-vertex structure means adjacent 2D triangles share 3D edges, producing a watertight mesh. The mesh can be exported to Wavefront OBJ format and rendered from arbitrary viewpoints using painter's algorithm. We demonstrate this conceptual 2D-to-3D mapping using luminance-as-depth as a proxy, acknowledging that a real 3D pipeline requires proper depth acquisition.
