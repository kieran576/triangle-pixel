...
**Layer 2 (Borrow).** Each triangle fills all three color channels from its three edge-sharing neighbors, excluding its own measurement from its own RGB output (which instead contributes to its three neighbors). Since the three neighbors collectively possess $\{R, G, B\}$ (Property 2), all channels can be populated:
...
**Visualization** (`demo_temporal_static.gif`, `demo_temporal_translating.gif`): Static-scene playback shows uniformly white frame-difference heatmap; translating-scene playback shows red highlights only at moving edges, with stationary background remaining white.
